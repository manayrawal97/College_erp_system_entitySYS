...
const FeatureCard = ({ icon: Icon, title, description, link, comingSoon, extra }) => {
  ...
  return (
    <motion.div
      ...
      className="relative p-6 bg-white dark:bg-gray-800 rounded-3xl shadow-md border border-gray-100 dark:border-gray-700 group transition-all hover:shadow-xl flex flex-col h-full"
    >
      {comingSoon && <ComingSoonBadge />}
      <div className="w-14 h-14 bg-blue-50 dark:bg-blue-900/30 rounded-2xl flex items-center justify-center mb-5 group-hover:scale-110 transition-transform text-secondary">
        <Icon className="h-7 w-7" />
      </div>
      <h3 className="text-lg font-black text-gray-900 dark:text-white mb-2 leading-tight">{title}</h3>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 line-clamp-2 flex-grow">{description}</p>
...
const NoticeSkeleton = () => (
  <div className="p-5 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 mb-4 animate-pulse">
...
const StudentDashboard = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-navy">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 font-black">Connecting to EntitySYS...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'dark' : ''} selection:bg-secondary selection:text-white`}>
      <div className="bg-[#F8FAFC] dark:bg-navy-dark transition-colors duration-300 min-h-screen flex flex-col">
        
        {/* --- Navbar --- */}
        <header className="sticky top-0 z-[100] bg-white/90 dark:bg-navy/90 backdrop-blur-xl border-b border-gray-200/50 dark:border-gray-800/50 px-4 md:px-8 py-4">
          <div className="container mx-auto flex justify-between items-center">
            ...
            <nav className="hidden lg:flex items-center space-x-10">
              <Link to="/about" className="text-sm font-black text-gray-600 dark:text-gray-300 hover:text-secondary transition-colors uppercase tracking-widest">About</Link>
              
              <div className="relative group">
                <button className="flex items-center space-x-2 text-sm font-black text-gray-600 dark:text-gray-300 hover:text-secondary transition-colors uppercase tracking-widest">
                  <span>Academics</span>
                  <ChevronDown className="h-4 w-4" />
                </button>
                <div className="absolute top-full left-0 mt-3 w-56 bg-white dark:bg-gray-800 shadow-2xl rounded-2xl p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all border border-gray-100 dark:border-gray-700 translate-y-2 group-hover:translate-y-0">
                  <Link to="/courses" className="block px-4 py-3 text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition-colors">Courses</Link>
                  <Link to="/timetable" className="block px-4 py-3 text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition-colors">Timetable</Link>
                  <Link to="/syllabus" className="block px-4 py-3 text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition-colors">Syllabus</Link>
                </div>
              </div>
              ...
              <div className="relative group">
                ...
                <div className="absolute top-full left-0 mt-3 w-56 bg-white dark:bg-gray-800 shadow-2xl rounded-2xl p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all border border-gray-100 dark:border-gray-700 translate-y-2 group-hover:translate-y-0">
                  <Link to="/clubs" className="block px-4 py-3 text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition-colors">Clubs</Link>
                  <Link to="/events" className="block px-4 py-3 text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition-colors">Events</Link>
                  <Link to="/campus" className="block px-4 py-3 text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition-colors">Campus</Link>
                </div>
              </div>

              <Link to="/contact" className="text-sm font-black text-gray-600 dark:text-gray-300 hover:text-secondary transition-colors uppercase tracking-widest">Contact</Link>
            </nav>

            {/* Right Side Actions */}
            <div className="flex items-center space-x-3 md:space-x-5">
              <button className="relative p-2.5 text-gray-500 hover:text-secondary transition-colors bg-gray-50 dark:bg-gray-800 rounded-xl">
                <Bell className="h-6 w-6" />
                <span className="absolute top-2 right-2 w-4 h-4 bg-red-500 text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white dark:border-gray-800">0</span>
              </button>

              <div className="relative">
                <button 
                  onClick={() => setIsProfileOpen(!isProfileOpen)}
                  className="flex items-center space-x-3 p-1.5 rounded-2xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-all border border-transparent hover:border-gray-200 dark:hover:border-gray-700"
                >
                  ...
                  <span className="hidden md:block text-sm font-black text-gray-900 dark:text-white">{user.full_name}</span>
                ...
                <AnimatePresence>
                  {isProfileOpen && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-4 w-72 bg-white dark:bg-gray-900 shadow-2xl rounded-3xl p-4 border border-gray-100 dark:border-gray-800 z-[110]"
                    >
                      <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl mb-4">
                        <p className="font-black text-gray-900 dark:text-white truncate">{user.full_name}</p>
                        <p className="text-xs text-gray-500 font-bold truncate mt-1">{user.email}</p>
                      </div>
                      <div className="space-y-1">
                        <Link to="/profile" className="flex items-center space-x-4 p-3.5 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm font-bold text-gray-600 dark:text-gray-300">
                          <User className="h-5 w-5 text-secondary" /> <span>My Profile</span>
                        </Link>
                        <Link to="/settings" className="flex items-center space-x-4 p-3.5 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm font-bold text-gray-600 dark:text-gray-300">
                          <Settings className="h-5 w-5 text-secondary" /> <span>Settings</span>
                        </Link>
                        <button 
                          onClick={() => setIsDarkMode(!isDarkMode)}
                          className="w-full flex items-center justify-between p-3.5 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm font-bold text-gray-600 dark:text-gray-300"
                        >
                          <div className="flex items-center space-x-4">
                            {isDarkMode ? <Sun className="h-5 w-5 text-secondary" /> : <Moon className="h-5 w-5 text-secondary" />}
                            <span>{isDarkMode ? 'Light Mode' : 'Dark Mode'}</span>
                          </div>
                          <div className={`w-11 h-6 rounded-full relative transition-colors ${isDarkMode ? 'bg-secondary' : 'bg-gray-200 dark:bg-gray-700'}`}>
                            <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm ${isDarkMode ? 'right-1' : 'left-1'}`}></div>
                          </div>
                        </button>
                        <div className="border-t border-gray-100 dark:border-gray-800 my-3 pt-3">
                          <button 
                            onClick={handleLogout}
                            className="w-full flex items-center space-x-4 p-3.5 rounded-2xl hover:bg-red-50 dark:hover:bg-red-900/20 text-red-500 transition-colors text-sm font-black"
                          >
        ...
        <main className="container mx-auto px-4 md:px-6 py-6 md:py-10 flex-grow">
          ...
          <div className="dashboard-grid">
            ...
            <div className="area-notices mb-6 md:mb-0">
              <div className="bg-white dark:bg-gray-800/40 rounded-[2.5rem] p-6 lg:p-10 h-full border border-gray-100 dark:border-gray-800 shadow-xl shadow-gray-200/50 dark:shadow-none flex flex-col">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10">
                  ...
                  <div className="flex flex-wrap gap-2 p-1.5 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800 self-start">
                    {['all', 'College', 'Exam', 'Event'].map((cat) => (
                      <button
                        key={cat}
                        onClick={() => setActiveCategory(cat.toLowerCase())}
                        className={`px-5 py-2.5 rounded-xl text-xs font-black transition-all relative ${
                ...
                <div className="space-y-5 flex-grow overflow-y-auto pr-3 custom-scrollbar min-h-[400px]">
                  <AnimatePresence mode="popLayout">
                    {noticesLoading ? (
                      ...
                      <div className="text-center py-20 bg-red-50 dark:bg-red-900/10 rounded-[2rem] border-2 border-dashed border-red-100 dark:border-red-900/20">
                        <AlertCircle className="h-12 w-12 text-red-500 mx-auto mb-4" />
                        <p className="text-red-600 dark:text-red-400 font-black mb-4">{noticesError}</p>
                        <button 
                      ...
                      notices.map((notice, index) => (
                        <motion.div
                          ...
                          className={`p-6 bg-white dark:bg-gray-800 rounded-3xl border transition-all cursor-pointer group shadow-sm hover:shadow-xl relative overflow-hidden ${
                            index === 0 ? 'border-secondary/50 ring-2 ring-secondary/5' : 'border-gray-100 dark:border-gray-700'
                          }`}
                        >
                          {index === 0 && (
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                            <div className="flex items-center gap-3">
                              <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-[0.15em] border ${
                                notice.category === 'Exam' ? 'bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-900/20 dark:border-rose-900/30' :
                                notice.category === 'Event' ? 'bg-indigo-50 text-indigo-600 border-indigo-100 dark:bg-indigo-900/20 dark:border-indigo-900/30' :
                                notice.category === 'College' ? 'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-900/20 dark:border-amber-900/30' :
                          ...
                          <h3 className="text-lg font-black text-gray-900 dark:text-white mb-2 group-hover:text-secondary transition-colors leading-tight">
                            {notice.title}
                          </h3>
                          <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-5 font-medium leading-relaxed">
                            {notice.content}
                          </p>
                          ...
                          <div className="flex items-center justify-between mt-auto">
                            ...
                            {(notice.hasAttachment || notice.attachment_path) && (
                              <div className="flex items-center gap-2 text-[10px] font-black text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 px-3 py-1.5 rounded-xl border border-emerald-100 dark:border-emerald-900/30">
                      ...
                      <div className="text-center py-24 bg-gray-50 dark:bg-gray-900/30 rounded-[2.5rem] border-2 border-dashed border-gray-200 dark:border-gray-800">
                        ...
                        <h4 className="text-lg font-black text-gray-900 dark:text-white mb-2">No Notices Available</h4>
          ...
          {selectedNotice && (
            <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
              ...
              >
                <div className="p-8 md:p-12">
                  <div className="flex justify-between items-start mb-8">
                    <div className="space-y-4">
                      ...
                      <h2 className="text-3xl font-black text-gray-900 dark:text-white leading-[1.1] tracking-tighter">
                  ...
                  <div className="prose dark:prose-invert max-w-none mb-10">
                    <p className="text-gray-600 dark:text-gray-300 leading-[1.6] font-bold text-base md:text-lg">
                      {selectedNotice.content}
                    </p>
                  </div>

                  {(selectedNotice.hasAttachment || selectedNotice.attachment_path) && (
                    <button className="flex items-center gap-5 w-full p-6 bg-emerald-50 dark:bg-emerald-900/10 border-2 border-dashed border-emerald-200 dark:border-emerald-900/30 rounded-3xl hover:border-emerald-500 hover:bg-emerald-50/80 transition-all group">
                      ...
                      <div className="text-left flex-grow">
                        <p className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-wider">Download Attachment</p>
                ...
                <div className="bg-gray-50 dark:bg-gray-800/50 p-6 md:p-8 flex justify-end gap-4">
                  <button 
                    onClick={() => setSelectedNotice(null)}
                    className="px-10 py-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl font-black text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all shadow-sm"
                  >
                    CLOSE
                  </button>
                  <button 
          ...
          {isMobileMenuOpen && (
            <>
              ...
              <motion.div
                ...
                className="fixed top-0 left-0 bottom-0 z-[130] w-80 bg-white dark:bg-navy p-8 lg:hidden overflow-y-auto"
              >
                <div className="flex justify-between items-center mb-12">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-2xl font-black text-primary dark:text-white tracking-tighter italic">EntitySYS</span>
                  </div>
                  <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-gray-50 dark:bg-gray-800 rounded-xl text-gray-400"><X /></button>
                </div>
                      ...
                      {['About', 'Courses', 'Timetable', 'Syllabus', 'Clubs', 'Events', 'Campus', 'Contact'].map(item => (
                        <li key={item}>
                          <Link 
                            to={`/${item.toLowerCase()}`} 
                            onClick={() => setIsMobileMenuOpen(false)}
                            className="text-2xl font-black text-gray-900 dark:text-white hover:text-secondary transition-colors block"
                          >
                            {item}
                          </Link>
                        </li>
