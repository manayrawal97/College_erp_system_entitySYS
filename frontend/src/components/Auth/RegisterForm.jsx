...
const RegisterForm = () => {
 ...
 className="w-full max-w-xl p-8 glass rounded-3xl shadow-2xl"
 >
 ...
 step >= i ? 'bg-secondary' : 'bg-gray-200 '
 }`}
 />
 ))}
 </div>
 ...
 >
 <p className="text-gray-600 text-center mb-4">Choose your role to get started</p>
 {roles.map((r) => (
 <button
 key={r.id}
 type="button"
 ...
 >
 <div className={`p-3 rounded-xl ${values.role === r.id ? 'bg-secondary text-white' : 'bg-gray-100 text-gray-400'}`}>
 ...
 type="button"
 onClick={prevStep}
 className="flex-1 px-6 py-3 border border-gray-300 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-gray-50 transition-colors"
 >
