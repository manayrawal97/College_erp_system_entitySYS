...
import { Bell, Plus, Pin, Trash2, Edit2, X, Search } from 'lucide-react';
...
const NoticeManagement = ({ isDashboard = false }) => {
    ...
    // Sort pinned notices to the top and filter by search query
    ...
    const filteredNotices = sortedNotices.filter(notice =>
        ...
        notice.target_role?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        notice.posted_by_name?.toLowerCase().includes(searchQuery.toLowerCase())
    );
    const displayedNotices = isDashboard ? filteredNotices.slice(0, 4) : filteredNotices;
            ...
            <div className="p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                {loading ? (
                    ...
                    <div className="col-span-full py-12 text-center text-gray-500 font-medium">
                        {searchQuery ? `No notices found matching "${searchQuery}".` : 'No notices posted yet.'}
                    </div>
                ) : (
                    displayedNotices.map((notice) => (
                        <div key={notice.id} className={`p-5 rounded-2xl border relative flex flex-col h-full transition-all ${
                            notice.is_pinned 
                                ? 'bg-amber-50/20 border-amber-200/60 shadow-sm' 
                            ...
                            <div className="flex items-center justify-between gap-2 mb-3">
                                ...
                                {Boolean(notice.is_pinned) && (
                                    <span className="flex items-center gap-1 text-[10px] font-bold text-amber-700 bg-amber-50 border border-amber-200/80 px-2 py-0.5 rounded-full shrink-0">
                                        <Pin size={11} fill="currentColor" />
                                        Pinned
                                    </span>
                                )}
                            </div>
                            <h3 className="font-bold text-gray-900 mb-2 leading-tight pr-2">{notice.title}</h3>
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-gray-200 mt-auto">
                                ...
                                <div className="flex items-center gap-1">
                                    <button 
                                        ...
                                        className={`p-1.5 transition-all rounded-lg min-h-[32px] min-w-[32px] cursor-pointer flex items-center justify-center ${
                                            notice.is_pinned 
                                                ? 'text-amber-600 bg-amber-100/80 hover:bg-amber-200/70 border border-amber-200' 
                                                : 'text-gray-400 hover:text-amber-600 hover:bg-gray-100 border border-transparent'
                                        }`} 
                                        title={notice.is_pinned ? 'Unpin Notice' : 'Pin Notice'}
                                    >
                                        <Pin size={15} fill={notice.is_pinned ? 'currentColor' : 'none'} />
                                    </button>
                                    <button 
                                        onClick={() => handleOpenEditNotice(notice)}
                                        className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 border border-transparent hover:border-blue-100 transition-all rounded-lg min-h-[32px] min-w-[32px] cursor-pointer flex items-center justify-center" 
                                        title="Edit Notice"
                                    >
                                        <Edit2 size={15} />
                                    </button>
                                    <button 
                                        ...
                                        className="p-1.5 text-gray-400 hover:text-rose-600 hover:bg-rose-50 border border-transparent hover:border-rose-100 transition-all rounded-lg min-h-[32px] min-w-[32px] cursor-pointer flex items-center justify-center" 
                                        title="Delete Notice"
                                    >
                                        <Trash2 size={15} />
                                    </button>
